...
def export_policy_as_jit_normal(networks, path, name="policy_1.pt"):
    os.makedirs(path, exist_ok=True)
    path = os.path.join(path, name)
    model = copy.deepcopy(networks).to("cpu")
    traced_script_module = torch.jit.script(model)
    traced_script_module.save(path)
    print("Exported policy as jit script to: ", path)


def convert_nn_to_onnx_normal(model, path, name):
    os.makedirs(path, exist_ok=True)
    path = os.path.join(path, name + ".onnx")
    policy = copy.deepcopy(model['network']).to("cpu")
    policy.eval()

    dummy_inputs = tuple(
        torch.randn(input_dim)
        for _, input_dim in model["forward_input_dims"].items()
    )
    if len(dummy_inputs) > 1:
        dummy_inputs = torch.cat(dummy_inputs, dim=-1)
    torch.onnx.export(
